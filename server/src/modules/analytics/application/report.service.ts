...
export class ReportService {
  ...
  async getTopActiveUsers(limit: number = 10): Promise<any> {
    ...
    return this.logModel.aggregate([
      { 
        $match: { 
          userId: { $exists: true, $nin: [null, 'system'] },
          timestamp: { $gte: dateLimit } 
        } 
      },
      { 
        ...
        $addFields: {
          crudType: {
            $switch: {
              branches: [
                { 
                  case: { $in: ['$context', ['deposit', 'transfer', 'withdraw']] }, 
                  then: 'CREATE' 
                },
                { 
                  case: { $in: ['$context', ['transaction-list']] }, 
                  then: 'READ' 
                },
                { 
                  case: { $in: ['$context', ['account-management', 'account', 'customer']] }, 
                  then: 'UPDATE' 
                }
              ],
              default: 'OTHER'
